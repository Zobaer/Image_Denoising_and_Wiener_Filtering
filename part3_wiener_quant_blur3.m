...
bwidth = 7; gsd = 1; mwidth= 9;
BIPB = imboxfilt(BI,bwidth);
BIPG = imgaussfilt(BI,gsd);
BIPM = medfilt2(BI, [mwidth mwidth],'symmetric');
psnr_BI = psnr(OI,BI); ssim_BI = ssim(OI,BI);
psnr_BIPB = psnr(OI,BIPB); ssim_BIPB = ssim(OI,BIPB);
psnr_BIPG = psnr(OI,BIPG); ssim_BIPG = ssim(OI,BIPG);
...
BIF=fftshift(fft2(BI));BIFPB=fftshift(fft2(BIPB));BIFPG=fftshift(fft2(BIPG));BIFPM=fftshift(fft2(BIPM));
a = 0.1; b = 0.1; T = 1; WF=ones(M,N); K = .02:.01:1;
for p=1:length(K)
    for u=1:M
         for v=1:N
    ...
    RDFB=BIFPB.*WF; RDIB=abs(ifft2(ifftshift(RDFB)));RDIB=RDIB/max(max(RDIB));
    ...
    psnr_FIB(p) = psnr(OI,RDIB); ssim_FIB(p) = ssim(OI,RDIB);
    psnr_FIG(p) = psnr(OI,RDIG); ssim_FIG(p) = ssim(OI,RDIG);
    psnr_FIM(p) = psnr(OI,RDIM); ssim_FIM(p) = ssim(OI,RDIM);
end

figure(2);subplot(121);plot(K,psnr_FI,LineWidth=1.4); hold on; grid on;
title("PSNR vs. K for Weiner filter",FontSize=f);
xlabel("K",FontSize=f);ylabel('PSNR (dB)',FontSize=f-4); 
plot(K,psnr_FIB,LineWidth=1.4);
plot(K,psnr_FIG,LineWidth=1.4);
plot(K,psnr_FIM,LineWidth=1.4);
legend("Without prefiltering","With box filtering","With Gauss filtering", "With median filtering",FontSize=f);

subplot(122);plot(K,ssim_FI,LineWidth=1.4); hold on;grid on;
title("SSIM vs. K for Weiner filter",FontSize=f);
xlabel("K",FontSize=f);ylabel('SSIM',FontSize=f-4); 
plot(K,ssim_FIB,LineWidth=1.4);
plot(K,ssim_FIG,LineWidth=1.4);
plot(K,ssim_FIM,LineWidth=1.4);
legend("Without prefiltering","With box filtering","With Gauss filtering", "With median filtering",FontSize=f);
...
% figure(3), imshow(RDI),
% title(sprintf('After Wiener FIltering with K = %.3f (PSNR=%.3fdB, SSIM=%.3f)',K, psnr_IF,ssim_IF),FontSize=f);